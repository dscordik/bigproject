...
export class BaseOAuthService {
    ...
    public async findUserByCode(code: string): Promise<TypeUserInfo> {
        ...
        if (!tokens.access_token) {
            throw new BadRequestException(`Нет токенов с ${this.options.profile_url}`)
        }
        ...
        if(!userRequest.ok) {
            throw new UnauthorizedException(`Не удалось получить пользователя ${this.options.profile_url}`)
        }
