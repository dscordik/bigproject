...
import {ResetPasswordDto} from "@/auth/password-recovery/dto/reset-password";
import {NewPasswordDto} from "@/auth/password-recovery/dto/new-password";
...
export class PasswordRecoveryService {
    ...
    public async newPassword(dto: NewPasswordDto, token: string) {
        ...
        if (!existingToken) {
            throw new NotFoundException(
                'Token не найден. Пожалуйста, проверьте правильность введенного токена или запросите новый.'
            );
        }
        ...
        if (hasExpired) {
            throw new BadRequestException('Токен истек. Пожалуйста запросите новый для сброса пароля');
        }
